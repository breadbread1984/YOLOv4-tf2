...
from models import YOLOv4, OutputParser;
...
class Predictor(object):
  ...
  def __init__(self, input_shape = (608,608,3), class_num = 80, yolov4 = None):
    ...
    if yolov4 is None:
      ...
      if exists("yolov4.h5"):
        ...
        self.yolov4 = tf.keras.models.load_model('yolov4.h5', compile = False);
      else:
        # restore from checkpoint
        self.yolov4 = YOLOv4(input_shape, class_num);
        optimizer = tf.keras.optimizers.Adam(1e-4);
        checkpoint = tf.train.Checkpoint(model = self.yolov4, optimizer = optimizer, optimizer_step = optimizer.iterations);
        checkpoint.restore(tf.train.latest_checkpoint('checkpoints'));
    else:
      self.input_shape = tuple(yolov4.input.shape[1:]);
      self.yolov4 = yolov4;
    self.parsers = [OutputParser(tuple(self.yolov4.outputs[l].shape[1:]), self.input_shape, self.anchors[l]) for l in range(3)];
  ...
  def predict(self, image, conf_thres = 0.5, nms_thres = 0.5):
    ...
    outputs = self.yolov4(images_data);
