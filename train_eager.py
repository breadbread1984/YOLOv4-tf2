...
def main():
  ...
  while True:
    ...
    with tf.GradientTape() as tape:
      outputs1, outputs2, outputs3 = yolov4(images);
    ...
    if tf.math.reduce_any(tf.math.is_nan(loss)) == True:
      ...
      continue;
    grads = tape.gradient(loss, yolov4.trainable_variables);
    ...
    if tf.math.reduce_any([tf.math.reduce_any(tf.math.is_nan(grad)) for grad in grads]) == True:
      ...
      continue;
    optimizer.apply_gradients(zip(grads, yolov4.trainable_variables));
